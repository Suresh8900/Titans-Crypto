...
const String LOGIN_END = "/api/v1/auth/login";
...
const String DRAFT_LEAD_DETAILS_END="/api/v1/leads/get-draft-details?lead_id=";
...
const String NOTIFICATION_API="notifications";
class ApiBaseService {
  ...
  Future<http.Response> get({String path = "", bool tokenRequired = false}) async {
  ...
  Future<http.Response> post({String body = "", String path = "", bool tokenRequired = false}) async {
    bool isConnected = await checkInternetConnectivity();
    if (!isConnected) {
      showToast('No internet connection');
      throw Exception('No internet connection');
  ...
  Future<http.Response> delete({String path = "",bool tokenRequired = false}) async {
    bool isConnected = await checkInternetConnectivity();
    if (!isConnected) {
      showToast('No internet connection');
      throw Exception('No internet connection');
  ...
  Future<http.Response> put({String body = "", String path = "",bool tokenRequired = false}) async {
    bool isConnected = await checkInternetConnectivity();
    if (!isConnected) {
      showToast('No internet connection');
      throw Exception('No internet connection');
